...
Exploration of the data showed that the division of users may not be so straightforward.
% TODO add plot of lengths vs time for different groups.

Figure plot shows how the lab and turk user classes cannot be separated by a simple metric of their data, such as length of events sequence and time to complete task.
...
Should be noted that length of events sequence and time are very correlated with a pearson's correlation of X.
This means that one of these features should be a good substitute for the other and that having both may be redundant.
